...
Architecture:
  ...
  FusionHead         : Concat(256-d) → BN → Linear(256) → ReLU → Dropout → Linear(num_classes)

Design rationale:
  - AdaptiveAvgPool makes the spatial resolution of the input irrelevant; the
    model accepts any H×W without recompilation (important for variable-size cameras).
  - BatchNorm before the fusion head stabilises training when the two encoders
...
class FusionBaseline(nn.Module):
    ...
    def forward(
        self, iris: torch.Tensor, fingerprint: torch.Tensor
    ) -> torch.Tensor:
